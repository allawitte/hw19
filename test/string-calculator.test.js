...
describe('String calculator should', function () {
    ...
    it('returns 3 if delimetrs are ; or ;\\n without // first', function(){
        var calc = createCalc();
        var sum = calc.intAdd(';\n1;2');
        assert.equal(sum,3);
    })
    it('returns negative numbers are not allowed when -1;2', function(){
        var calc = createCalc();
        var sum = calc.intAdd('-1;2');
        assert.equal(sum,'negative numbers are not allowed');
    })
